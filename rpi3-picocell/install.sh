...
# Build lora-picoGW_packet_forwarder-forwarder if not install by default
echo "picoGW_packet_forwarder building ..."

mkdir -p  Lora-net
cd Lora-net
git clone https://github.com/Lora-net/picoGW_hal.git
(cd picoGW_hal; make clean; make)

git clone https://github.com/Lora-net/picoGW_packet_forwarder.git
(cd picoGW_packet_forwarder; make clean; make)

echo "picoGW_packet_forwarder built"
...
wget $REPO/rpi3-picocell/lora-packet-forwarder/local_conf.json -O local_conf.LOCALHOST.json
sed -i.bak s/__ANTENNA_GAIN_DBI__/$ANTENNA_GAIN_DBI/g local_conf.LOCALHOST.json

wget $REPO/rpi3-picocell/lora-packet-forwarder/start.sh -O start.sh
wget $REPO/rpi3-picocell/lora-packet-forwarder/stop.sh -O stop.sh
wget $REPO/rpi3-picocell/lora-packet-forwarder/reset_lgw.sh -O reset_lgw.sh
chmod +x *.sh

sudo apt update
sudo apt install monit -y
wget $REPO/rpi3-picocell/lora-packet-forwarder/monitrc -O monitrc
